...
- Parsing and translation: 
    ...
    - parser检查语法，确认关系
- Optimization: 选择开销最低的evaluation plan
- Evaluation： 
...
!!! Example
    ...
    逻辑优化：把选择运算往叶子上推,尽量先做；先连接的是结果比较小的。使得中间结果比较小，能够减少开销
    An **evaluation plan** defines exactly what algorithm is used for each operation, and how the execution of the operations is coordinated.  每个操作都有很多种算法
...
* $t_T$ – time to transfer one block（不区分读写）
...
binary search generally does not make sense since data is not stored consecutively
### 3.2 Index scan
